#include "canny.h"
#include "ui_canny.h"
#include <math.h>
#include <iostream>

CannyDetection::CannyDetection(cv::Mat *src, QWidget *parent) :
	QMainWindow(parent),
	ui(new Ui::CannyDetection)
{
	ui->setupUi(this);

	cv::cvtColor((*src), img, CV_BGR2GRAY);
	//(*src).copyTo(img);
	detectEdges();
}

CannyDetection::~CannyDetection()
{
	delete ui;
}

void CannyDetection::detectEdges() {
	cv::Mat edges;
	cv::Mat blured;
	cv::GaussianBlur(img, blured, cv::Size(5, 5), 1.4);

	cv::Mat Gx, Gy;
	cv::Sobel(blured, Gx, CV_16S, 1, 0);
	cv::Sobel(blured, Gy, CV_16S, 0, 1);

	cv::Mat G, direction;
	G.create(Gx.size(), CV_16S);
	direction.create(Gx.size(), CV_16S);
	for (int i = 0; i < G.rows; i++) {
		for (int j = 0; j < G.cols; j++) {
			double Gxi = Gx.at<double>(i, j);
			double Gyi = Gy.at<double>(i, j);

			G.row(i).col(j) = sqrt(Gxi * Gxi + Gyi * Gyi);

			double angle = (atan2(Gxi, Gyi) / M_PI) * 180;
			if ((angle > -22.5 && angle < 22.5) || angle > 157.5 || angle < -157.5) {
				direction.at<int>(i, j) = 0;
			} else if ((angle > 22.5 && angle < 67.5) || (angle > -157.5 && angle < -112.5)) {
				direction.at<int>(i, j) = 45;
			} else if ((angle > 67.5 && angle < 112.5) || (angle > -112.5 && angle < -67.5)) {
				direction.at<int>(i, j) = 90;
			} else {
				direction.at<int>(i, j) = 135;
			}
		}
	}

	for (int i = 0; i < G.rows; i++) {
		for (int j = 0; j < G.cols; j++) {
			int max = 1;
			if (direction.at<int>(i, j) == 0) {
				if (j == 0) {
					if (G.at<double>(i, j) < G.at<double>(i, j + 1)) max = 0;
				} else if (j == G.cols - 1) {
					if (G.at<double>(i, j) < G.at<double>(i, j - 1)) max = 0;
				} else {
					if (G.at<double>(i, j) < G.at<double>(i, j - 1) || G.at<double>(i, j) < G.at<double>(i, j + 1)) max = 0;
				}
			} else if (direction.at<int>(i, j) == 45) {
				if ((i == 0 && j == 0) || (i == G.rows - 1 && j == G.cols - 1)) {
					continue;
				} else if (i == 0 || j == G.cols - 1) {
					if (G.at<double>(i, j) < G.at<double>(i + 1, j - 1)) max = 0;
				} else if (i == G.rows - 1 || j == 0) {
					if (G.at<double>(i, j) < G.at<double>(i - 1, j + 1)) max = 0;
				} else {
					if (G.at<double>(i, j) < G.at<double>(i - 1, j + 1) || G.at<double>(i, j) < G.at<double>(i + 1, j - 1)) max = 0;
				}
			} else if (direction.at<int>(i, j) == 90) {
				if (i == 0) {
					if (G.at<double>(i, j) < G.at<double>(i + 1, j)) max = 0;
				} else if (i == G.rows - 1) {
					if (G.at<double>(i, j) < G.at<double>(i - 1, j)) max = 0;
				} else {
					if (G.at<double>(i, j) < G.at<double>(i - 1, j) || G.at<double>(i, j) < G.at<double>(i + 1, j)) max = 0;
				}
			} else {
				if ((i == 0 && j == G.cols - 1) || (i == G.rows - 1 && j == 0)) {
					continue;
				} else if (i == 0 || j == 0) {
					if (G.at<double>(i, j) < G.at<double>(i + 1, j + 1)) max = 0;
				} else if (i == G.rows - 1 || j == G.cols - 1) {
					if (G.at<double>(i, j) < G.at<double>(i - 1, j - 1)) max = 0;
				} else {
					if (G.at<double>(i, j) < G.at<double>(i - 1, j - 1) || G.at<double>(i, j) < G.at<double>(i + 1, j + 1)) max = 0;
				}
			}

			if (!max) {
				G.at<double>(i, j) = 0;
			}
		}
	}
	edges.create(blured.size(), blured.type());

	QPixmap p = QPixmap::fromImage(QImage((unsigned char*) G.data, G.cols, G.rows, QImage::Format_Indexed8));
	//QPixmap p = QPixmap::fromImage(QImage((unsigned char*) edges.data, edges.cols, edges.rows, QImage::Format_RGB888).rgbSwapped());
	ui->label->setPixmap(p);
	ui->label->setFixedSize(p.size());
	ui->centralwidget->setFixedSize(p.size());
}
